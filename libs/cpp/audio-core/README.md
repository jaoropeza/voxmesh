...
Interfaces not yet defined (`IClockSynchronizer`, `IAudioResampler`, `IAudioProcessor`,
`IAudioEncoder`, `IRecordingWriter`, `IStreamingAudioClient`, `IConsentService`) arrive with the
phases that first implement them; drift correction (ADR-0007) lands with Phase 2 clock sync.
